 ...
 * Description: sample_comm_audio.c
...
#ifdef HI_ACODEC_TYPE_ES8388
#include "es8388.h"
#endif
#ifdef HI_ACODEC_TYPE_NVP6158
#include "common.h"
#endif
#ifdef HI_ACODEC_TYPE_TP2828
#include "tp2802.h"
#endif
...
{
    hi_s32 ret;
    hi_es8388_audio_ctrl audio_ctrl;
    audio_ctrl.chip_num = ES8388_CHIP_ID;

    ret = ioctl(codec_fd, HI_ES8388_SOFT_RESET, &audio_ctrl);
    if (ret != HI_SUCCESS) {
...
hi_s32 sample_es8388_set_input_ctrl(int codec_fd)
{
    ...
    /* set input select */
    audio_ctrl.audio_in_select = ES8388_INPUT1; /* refer to hardware */
    ret = ioctl(codec_fd, HI_ES8388_SET_LEFT_INPUT_SELECT, &audio_ctrl);
    if (ret != HI_SUCCESS) {
        printf("[Func]:%s [Line]:%d [Info]:%s\n", __FUNCTION__, __LINE__, "es83888 set left input fail.");
        return ret;
    }
    audio_ctrl.audio_in_select = ES8388_INPUT1; /* refer to hardware */
    ret = ioctl(codec_fd, HI_ES8388_SET_RIGHT_INPUT_SELECT, &audio_ctrl);
    if (ret != HI_SUCCESS) {
    ...
    ret = ioctl(codec_fd, HI_ES8388_SET_LFET_INPUT_POWER, &audio_ctrl);
    if (ret != HI_SUCCESS) {
        printf("[Func]:%s [Line]:%d [Info]:%s\n", __FUNCTION__, __LINE__, "es83888 set left in power fail.");
        return ret;
    }
    ret = ioctl(codec_fd, HI_ES8388_SET_RIGHT_INPUT_POWER, &audio_ctrl);
    if (ret != HI_SUCCESS) {
    ...
    /* power on ADC */
    audio_ctrl.if_powerup = ES8388_POWER_UP;
    ret = ioctl(codec_fd, HI_ES8388_SET_LFET_ADC_POWER, &audio_ctrl);
    if (ret != HI_SUCCESS) {
        printf("[Func]:%s [Line]:%d [Info]:%s\n", __FUNCTION__, __LINE__, "es83888 set left ADC power fail.");
        return ret;
    }
    ret = ioctl(codec_fd, HI_ES8388_SET_RIGHT_ADC_POWER, &audio_ctrl);
    if (ret != HI_SUCCESS) {
...
hi_s32 sample_es8388_set_clk_mode(int codec_fd, hi_aio_mode work_mode, hi_audio_sample_rate sample_rate)
{
    ...
    if (work_mode == HI_AIO_MODE_I2S_SLAVE || work_mode == HI_AIO_MODE_PCM_SLAVE_STD ||
        ...
        ret = ioctl(codec_fd, HI_ES8388_SET_MCLK_BCLK_RATIO, &audio_ctrl);
        if (ret != HI_SUCCESS) {
        ...
        ret = ioctl(codec_fd, HI_ES8388_SET_ADC_MCLK_LRCLK_RATIO, &audio_ctrl);
        if (ret != HI_SUCCESS) {
            printf("[Func]:%s [Line]:%d [Info]:%s\n", __FUNCTION__, __LINE__, "es83888 set ADC LRCLK ratio fail.");
            return ret;
        }
        ...
        audio_ctrl.clk_div = fsclk_ratio;
        ret = ioctl(codec_fd, HI_ES8388_SET_DAC_MCLK_LRCLK_RATIO, &audio_ctrl);
        if (ret != HI_SUCCESS) {
    ...
    ret = ioctl(codec_fd, HI_ES8388_SET_MASTER_MODE, &audio_ctrl);
    if (ret != HI_SUCCESS) {
...
hi_s32 sample_es8388_set_clk_polarity(int codec_fd, hi_aio_mode work_mode)
{
    ...
    ret = ioctl(codec_fd, HI_ES8388_SET_ADC_POLARITY_AND_OFFSET, &audio_ctrl);
    if (ret != HI_SUCCESS) {
        printf("[Func]:%s [Line]:%d [Info]:%s\n", __FUNCTION__, __LINE__, "es83888 set ADC polarity or offset fail.");
        return ret;
    }
    ret = ioctl(codec_fd, HI_ES8388_SET_DAC_POLARITY_AND_OFFSET, &audio_ctrl);
    if (ret != HI_SUCCESS) {
    ...
    ret = ioctl(codec_fd, HI_ES8388_SET_BCLK_DIR, &audio_ctrl);
    if (ret != HI_SUCCESS) {
...
hi_s32 sample_es8388_set_data_ctrl(int codec_fd, hi_aio_mode work_mode, hi_audio_bit_width bit_width)
{
    ...
    ret = ioctl(codec_fd, HI_ES8388_SET_ADC_DATA_FORMAT, &audio_ctrl);
    if (ret != HI_SUCCESS) {
        printf("[Func]:%s [Line]:%d [Info]:%s\n", __FUNCTION__, __LINE__, "es83888 set ADC format fail.");
        return ret;
    }
    ret = ioctl(codec_fd, HI_ES8388_SET_DAC_DATA_FORMAT, &audio_ctrl);
    if (ret != HI_SUCCESS) {
    ...
    ret = ioctl(codec_fd, HI_ES8388_SET_ADC_DATA_WIDTH, &audio_ctrl);
    if (ret != HI_SUCCESS) {
        printf("[Func]:%s [Line]:%d [Info]:%s\n", __FUNCTION__, __LINE__, "es83888 set ADC width fail.");
        return ret;
    }
    ret = ioctl(codec_fd, HI_ES8388_SET_DAC_DATA_WIDTH, &audio_ctrl);
    if (ret != HI_SUCCESS) {
...
hi_s32 sample_es8388_set_output_ctrl(int codec_fd)
{
    ...
    /* power on DAC */
    audio_ctrl.if_powerup = ES8388_POWER_UP;
    ret = ioctl(codec_fd, HI_ES8388_SET_LFET_DAC_POWER, &audio_ctrl);
    if (ret != HI_SUCCESS) {
        printf("[Func]:%s [Line]:%d [Info]:%s\n", __FUNCTION__, __LINE__, "es83888 set left DAC power fail.");
        return ret;
    }
    ret = ioctl(codec_fd, HI_ES8388_SET_RIGHT_DAC_POWER, &audio_ctrl);
    if (ret != HI_SUCCESS) {
    ...
    ret = ioctl(codec_fd, HI_ES8388_SET_LFET_OUTPUT1_ENABLE, &audio_ctrl);
    if (ret != HI_SUCCESS) {
        printf("[Func]:%s [Line]:%d [Info]:%s\n", __FUNCTION__, __LINE__, "es83888 set left out1 enable fail.");
        return ret;
    }
    ret = ioctl(codec_fd, HI_ES8388_SET_RIGHT_OUTPUT1_ENABLE, &audio_ctrl);
    if (ret != HI_SUCCESS) {
        printf("[Func]:%s [Line]:%d [Info]:%s\n", __FUNCTION__, __LINE__, "es83888 set right out1 enable fail.");
        return ret;
    }
    ret = ioctl(codec_fd, HI_ES8388_SET_LFET_OUTPUT2_ENABLE, &audio_ctrl);
    if (ret != HI_SUCCESS) {
        printf("[Func]:%s [Line]:%d [Info]:%s\n", __FUNCTION__, __LINE__, "es83888 set left out2 enable fail.");
        return ret;
    }
    ret = ioctl(codec_fd, HI_ES8388_SET_RIGHT_OUTPUT2_ENABLE, &audio_ctrl);
    if (ret != HI_SUCCESS) {
...
hi_s32 sample_es8388_set_input_volume(int codec_fd)
{
    ...
    ret = ioctl(codec_fd, HI_ES8388_SET_LFET_INPUT_GAIN, &audio_ctrl);
    if (ret != HI_SUCCESS) {
        printf("[Func]:%s [Line]:%d [Info]:%s\n", __FUNCTION__, __LINE__, "es83888 set left gain fail.");
        return ret;
    }
    ret = ioctl(codec_fd, HI_ES8388_SET_RIGHT_INPUT_GAIN, &audio_ctrl);
    if (ret != HI_SUCCESS) {
    ...
    ret = ioctl(codec_fd, HI_ES8388_SET_LFET_ADC_VOLUME, &audio_ctrl);
    if (ret != HI_SUCCESS) {
        printf("[Func]:%s [Line]:%d [Info]:%s\n", __FUNCTION__, __LINE__, "es83888 set left ADC vol fail.");
        return ret;
    }
    ret = ioctl(codec_fd, HI_ES8388_SET_RIGHT_ADC_VOLUME, &audio_ctrl);
    if (ret != HI_SUCCESS) {
    ...
    ret = ioctl(codec_fd, HI_ES8388_SET_ADC_MUTE, &audio_ctrl);
    if (ret != HI_SUCCESS) {
...
hi_s32 sample_es8388_set_output_volume(int codec_fd)
{
    ...
    ret = ioctl(codec_fd, HI_ES8388_SET_LFET_OUTPUT1_VOLUME, &audio_ctrl);
    if (ret != HI_SUCCESS) {
        printf("[Func]:%s [Line]:%d [Info]:%s\n", __FUNCTION__, __LINE__, "es83888 set left out1 vol fail.");
        return ret;
    }
    ret = ioctl(codec_fd, HI_ES8388_SET_RIGHT_OUTPUT1_VOLUME, &audio_ctrl);
    if (ret != HI_SUCCESS) {
        printf("[Func]:%s [Line]:%d [Info]:%s\n", __FUNCTION__, __LINE__, "es83888 set right out1 vol fail.");
        return ret;
    }
    ret = ioctl(codec_fd, HI_ES8388_SET_LFET_OUTPUT2_VOLUME, &audio_ctrl);
    if (ret != HI_SUCCESS) {
        printf("[Func]:%s [Line]:%d [Info]:%s\n", __FUNCTION__, __LINE__, "es83888 set left out2 vol fail.");
        return ret;
    }
    ret = ioctl(codec_fd, HI_ES8388_SET_RIGHT_OUTPUT2_VOLUME, &audio_ctrl);
    if (ret != HI_SUCCESS) {
    ...
    ret = ioctl(codec_fd, HI_ES8388_SET_LFET_DAC_VOLUME, &audio_ctrl);
    if (ret != HI_SUCCESS) {
        printf("[Func]:%s [Line]:%d [Info]:%s\n", __FUNCTION__, __LINE__, "es83888 set left DAC vol fail.");
        return ret;
    }
    ret = ioctl(codec_fd, HI_ES8388_SET_RIGHT_DAC_VOLUME, &audio_ctrl);
    if (ret != HI_SUCCESS) {
    ...
    /* set DAC unmute */
    audio_ctrl.if_mute = ES8388_MUTE_DISABLE;
    ret = ioctl(codec_fd, HI_ES8388_SET_DAC_MUTE, &audio_ctrl);
    if (ret != HI_SUCCESS) {
...
/* destory the thread to get frame from ai and send to ao or aenc */
...
/* destory the thread to get stream from aenc and send to adec */
...
/* destory the thread to get stream from file and send to adec */
...
/* destory the thread to set ao volume */
    ...
    const sample_comm_ai_vqe_param *ai_vqe_param, hi_audio_dev ao_dev_id)
{
    ...
    for (i = 0; i < (ai_chn_cnt >> ((hi_u32)aio_attr->snd_mode)); i++) {
...
static hi_s32 hdmi_set_audio_param(const hi_aio_attr *aio_attr, hi_hdmi_attr *hdmi_attr)
{
   /* if enable audio */
    hdmi_attr->audio_en = HI_TRUE;

    /* sampling rate of PCM audio, the parameter must be consistent with the input of AO */
    switch (aio_attr->sample_rate) {
        case HI_AUDIO_SAMPLE_RATE_32000:
            hdmi_attr->sample_rate = HI_HDMI_SAMPLE_RATE_32K;
            break;

        case HI_AUDIO_SAMPLE_RATE_44100:
            hdmi_attr->sample_rate = HI_HDMI_SAMPLE_RATE_44K;
            break;

        case HI_AUDIO_SAMPLE_RATE_48000:
            hdmi_attr->sample_rate = HI_HDMI_SAMPLE_RATE_48K;
            break;
    ...
    switch (aio_attr->bit_width) {
        case HI_AUDIO_BIT_WIDTH_8:
            hdmi_attr->bit_depth = HI_HDMI_BIT_DEPTH_8;
            break;

        case HI_AUDIO_BIT_WIDTH_16:
            hdmi_attr->bit_depth = HI_HDMI_BIT_DEPTH_16;
            break;

        case HI_AUDIO_BIT_WIDTH_24:
            hdmi_attr->bit_depth = HI_HDMI_BIT_DEPTH_24;
            break;
...
hi_s32 sample_comm_audio_start_hdmi(hi_aio_attr *aio_attr)
{
    hi_s32 ret;
    hi_hdmi_attr hdmi_attr;
    hi_hdmi_id hdmi = HI_HDMI_ID_0;
    hi_vo_pub_attr pub_attr;
    hi_vo_dev vo_dev = 0;

    pub_attr.bg_color = 0x000000ff;
    pub_attr.intf_type = HI_VO_INTF_HDMI;
    pub_attr.intf_sync = HI_VO_OUT_1080P30;

    if (sample_comm_vo_start_dev(vo_dev, &pub_attr) != HI_SUCCESS) {
    ...
    ret = hi_mpi_hdmi_stop(hdmi);
    if (ret != HI_SUCCESS) {
        printf("[func]:%s [line]:%d [info]:%s\n", __FUNCTION__, __LINE__, "hi_mpi_hdmi_stop failed");
        return HI_FAILURE;
    }

    ret = hi_mpi_hdmi_get_attr(hdmi, &hdmi_attr);
    if (ret != HI_SUCCESS) {
        printf("[func]:%s [line]:%d [info]:%s\n", __FUNCTION__, __LINE__, "hi_mpi_hdmi_get_attr failed");
        return HI_FAILURE;
    }
    ...
    ret = hi_mpi_hdmi_set_attr(hdmi, &hdmi_attr);
    if (ret != HI_SUCCESS) {
        printf("[func]:%s [line]:%d [info]:%s\n", __FUNCTION__, __LINE__, "hi_mpi_hdmi_set_attr failed");
        return HI_FAILURE;
    }

    ret = hi_mpi_hdmi_start(hdmi);
    if (ret != HI_SUCCESS) {
        printf("[func]:%s [line]:%d [info]:%s\n", __FUNCTION__, __LINE__, "hi_mpi_hdmi_start failed");
        return HI_FAILURE;
    }
...
hi_s32 sample_comm_audio_stop_hdmi(hi_void)
{
    hi_s32 ret;
    hi_vo_dev vo_dev = 0;
    ...
    hi_audio_sample_rate in_sample_rate, hi_bool resample_en)
{
    ...
    for (i = 0; i < (ao_chn_cnt >> ((hi_u32)aio_attr->snd_mode)); i++) {
        ...
        if (resample_en == HI_TRUE) {
            ...
            if (ret != HI_SUCCESS) {
                printf("%s: hi_mpi_ao_enable_re_smp(%d,%d) failed with %#x!\n", __FUNCTION__, ao_dev_id, i, ret);
                return HI_FAILURE;
            }
        }
    }
...
hi_s32 sample_comm_audio_destory_all_trd(void)
{
    ...
    for (dev_id = 0; dev_id < HI_AI_DEV_MAX_NUM; dev_id++) {
        for (chn_id = 0; chn_id < HI_AI_MAX_CHN_NUM; chn_id++) {
            if (sample_comm_audio_destory_trd_ai(dev_id, chn_id) != HI_SUCCESS) {
    ...
    for (chn_id = 0; chn_id < HI_AENC_MAX_CHN_NUM; chn_id++) {
        if (sample_comm_audio_destory_trd_aenc_adec(chn_id) != HI_SUCCESS) {
    ...
    for (chn_id = 0; chn_id < HI_ADEC_MAX_CHN_NUM; chn_id++) {
        if (sample_comm_audio_destory_trd_file_adec(chn_id) != HI_SUCCESS) {
    ...
    for (chn_id = 0; chn_id < HI_AO_DEV_MAX_NUM; chn_id++) {
        if (sample_comm_audio_destory_trd_ao_vol_ctrl(chn_id) != HI_SUCCESS) {
...
hi_s32 sample_comm_audio_start_adec(hi_u32 adec_chn_cnt, hi_payload_type type)
{
    ...
    adec_attr.mode = HI_ADEC_MODE_STREAM; /* propose use pack mode in your app */
    ...
    if (adec_attr.type == HI_PT_ADPCMA) {
        ...
        adec_attr.mode = HI_ADEC_MODE_STREAM; /* aac should be stream mode */
