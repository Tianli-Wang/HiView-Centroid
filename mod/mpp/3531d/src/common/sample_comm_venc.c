...
typedef struct {
    ...
    hi_u64 qpmap_phys_addr[VENC_QPMAP_MAX_CHN][QPMAP_BUF_NUM];
    hi_void *qpmap_vir_addr[VENC_QPMAP_MAX_CHN][QPMAP_BUF_NUM];

    hi_u32 skip_weight_size[VENC_QPMAP_MAX_CHN];
    hi_u64 skip_weight_phys_addr[VENC_QPMAP_MAX_CHN][QPMAP_BUF_NUM];
...
static hi_s32 read_jpg_file_pos(FILE *fp_jpg, hi_char *psz_file, hi_s32 size, hi_s32 *startpos, hi_s32 *endpos)
{
    ...
    hi_char startflag[2] = { 0xff, 0xd8 }; /* 2 is array size */
    hi_char endflag[2] = { 0xff, 0xd9 }; /* 2 is array size */
...
static hi_void file_trans_set_tembuf(FILE *fp_jpg, struct stat *stat_info, hi_s32 *startpos, hi_s32 *endpos)
{
    ...
    hi_s32 fd;
    hi_s32 bufpos = 0;
    hi_char startflag[2] = { 0xff, 0xd8 }; /* 2 is a number */
    hi_char endflag[2] = { 0xff, 0xd9 }; /* 2 is a number */
    ...
    while (!feof(fp_jpg)) {
        ...
        if (bufpos > 0) {
            if (memcmp(tempbuf + bufpos - 1, startflag, sizeof(startflag)) == 0) {
                *startpos = ((ftell(fp_jpg) - 2) < 0) ? 0 : ((ftell(fp_jpg) - 2) < 0); /* 2 is a number 2 is a number */
                start_match = HI_TRUE;
            }
...
hi_s32 sample_comm_venc_mem_config(hi_void)
{
    ...
    for (i = 0; i < 64; i++) {
        ...
        if (ret != HI_SUCCESS) {
            SAMPLE_PRT("hi_mpi_sys_set_mem_config with %#x!\n", ret);
            return HI_FAILURE;
        }
    }
    return HI_SUCCESS;
...
hi_s32 sample_comm_venc_get_dcf_info(hi_char *src_jpg_path, hi_u8 src_len, hi_char *dst_thm_path, hi_u8 dst_len)
{
    hi_s32 rtn_val, fd;
    hi_char jpg_src_path[FILE_NAME_LEN] = {0};
    hi_char jpg_des_path[FILE_NAME_LEN] = {0};
    hi_u32 dst_size = 0;
    if (snprintf_s(jpg_src_path, sizeof(jpg_src_path), sizeof(jpg_src_path) - 1, "%s", src_jpg_path) < 0) {
        return HI_FAILURE;
    }

    if (snprintf_s(jpg_des_path, sizeof(jpg_des_path), sizeof(jpg_des_path) - 1, "%s", dst_thm_path) < 0) {
        return HI_FAILURE;
    }
...
static hi_void sample_comm_venc_mjpeg_vbr_param_init(hi_venc_chn_attr *venc_chn_attr, hi_u32 gop, hi_u32 stats_time,
    hi_u32 frame_rate, hi_pic_size size)
{
...
static hi_void sample_comm_venc_mjpeg_cbr_param_init(hi_venc_chn_attr *venc_chn_attr, hi_u32 gop, hi_u32 stats_time,
    hi_u32 frame_rate, hi_pic_size size)
{
...
static hi_s32 sample_comm_venc_mjpeg_param_init(hi_venc_chn_attr *venc_chn_attr,
    sample_comm_venc_chnl_param *venc_create_chn_param)
{
    sample_rc rc_mode    = venc_create_chn_param->rc_mode;
    hi_u32 gop           = venc_create_chn_param->gop;
    ...
    if (rc_mode == SAMPLE_RC_FIXQP) {
        ...
        sample_comm_venc_mjpeg_cbr_param_init(venc_chn_attr, gop, stats_time, frame_rate, size);
        ...
        sample_comm_venc_mjpeg_vbr_param_init(venc_chn_attr, gop, stats_time, frame_rate, size);
    } else {
...
static hi_s32 sample_comm_venc_h264_param_init(hi_venc_chn_attr *venc_chn_attr,
    sample_comm_venc_chnl_param *venc_create_chn_param)
{
    sample_rc rc_mode    = venc_create_chn_param->rc_mode;
    hi_u32 gop           = venc_create_chn_param->gop;
    ...
    venc_chn_attr->venc_attr.h264_attr.frame_buf_ratio = SAMPLE_FRAME_BUF_RATIO_MAX;
    if (rc_mode == SAMPLE_RC_CBR) {
        sample_comm_venc_h264_cbr_param_init(venc_chn_attr, gop, stats_time, frame_rate, size);
    } else if (rc_mode == SAMPLE_RC_FIXQP) {
        sample_comm_venc_h264_fixqp_param_init(venc_chn_attr, gop, frame_rate);
    } else if (rc_mode == SAMPLE_RC_VBR) {
        sample_comm_venc_h264_vbr_param_init(venc_chn_attr, gop, stats_time, frame_rate, size);
    } else if (rc_mode == SAMPLE_RC_AVBR) {
        sample_comm_venc_h264_avbr_param_init(venc_chn_attr, gop, stats_time, frame_rate, size);
    } else if (rc_mode == SAMPLE_RC_CVBR) {
        sample_comm_venc_h264_cvbr_param_init(venc_chn_attr, gop, stats_time, frame_rate, size);
    } else if (rc_mode == SAMPLE_RC_QVBR) {
        sample_comm_venc_h264_qvbr_param_init(venc_chn_attr, gop, stats_time, frame_rate, size);
    } else if (rc_mode == SAMPLE_RC_QPMAP) {
        sample_comm_venc_h264_qpmap_param_init(venc_chn_attr, gop, frame_rate, stats_time);
    } else {
        SAMPLE_PRT("%s,%d,rc_mode(%d) not support\n", __FUNCTION__, __LINE__, rc_mode);
        return HI_FAILURE;
    }
    venc_chn_attr->venc_attr.h264_attr.rcn_ref_share_buf_en = venc_create_chn_param->is_rcn_ref_share_buf;
    return HI_SUCCESS;
}

static hi_s32 sample_comm_venc_h265_param_init(hi_venc_chn_attr *venc_chn_attr,
    sample_comm_venc_chnl_param *venc_create_chn_param)
{
    sample_rc rc_mode    = venc_create_chn_param->rc_mode;
    hi_u32 gop           = venc_create_chn_param->gop;
    ...
    venc_chn_attr->venc_attr.h265_attr.frame_buf_ratio = SAMPLE_FRAME_BUF_RATIO_MAX;
    if (rc_mode == SAMPLE_RC_CBR) {
        sample_comm_venc_h265_cbr_param_init(venc_chn_attr, gop, stats_time, frame_rate, size);
    } else if (rc_mode == SAMPLE_RC_FIXQP) {
        sample_comm_venc_h265_fixqp_param_init(venc_chn_attr, gop, frame_rate);
    } else if (rc_mode == SAMPLE_RC_VBR) {
        sample_comm_venc_h265_vbr_param_init(venc_chn_attr, gop, stats_time, frame_rate, size);
    } else if (rc_mode == SAMPLE_RC_AVBR) {
        sample_comm_venc_h265_avbr_param_init(venc_chn_attr, gop, stats_time, frame_rate, size);
    } else if (rc_mode == SAMPLE_RC_CVBR) {
        sample_comm_venc_h265_cvbr_param_init(venc_chn_attr, gop, stats_time, frame_rate, size);
    } else if (rc_mode == SAMPLE_RC_QVBR) {
        sample_comm_venc_h265_qvbr_param_init(venc_chn_attr, gop, stats_time, frame_rate, size);
    } else if (rc_mode == SAMPLE_RC_QPMAP) {
        sample_comm_venc_h265_qpmap_param_init(venc_chn_attr, gop, frame_rate, stats_time);
    } else {
        SAMPLE_PRT("%s,%d,rc_mode(%d) not support\n", __FUNCTION__, __LINE__, rc_mode);
        return HI_FAILURE;
    }
    venc_chn_attr->venc_attr.h265_attr.rcn_ref_share_buf_en = venc_create_chn_param->is_rcn_ref_share_buf;

    return HI_SUCCESS;
}

static hi_void sample_comm_venc_set_gop_attr(hi_payload_type type, hi_venc_chn_attr *venc_chn_attr,
    hi_venc_gop_attr *gop_attr)
{
    if (type == HI_PT_MJPEG || type == HI_PT_JPEG) {
        venc_chn_attr->gop_attr.gop_mode = HI_VENC_GOP_MODE_NORMAL_P;
        venc_chn_attr->gop_attr.normal_p.ip_qp_delta = 0;
    } else {
        venc_chn_attr->gop_attr = *gop_attr;
        if ((gop_attr->gop_mode == HI_VENC_GOP_MODE_BIPRED_B) && (type == HI_PT_H264)) {
            if (venc_chn_attr->venc_attr.profile == 0) {
                venc_chn_attr->venc_attr.profile = 1;
...
static hi_s32 sample_comm_venc_channel_param_init(sample_comm_venc_chnl_param *venc_create_chn_param,
    hi_venc_chn_attr *venc_chn_attr)
{
    hi_s32 ret;
    hi_venc_gop_attr *gop_attr          = &venc_create_chn_param->gop_attr;
    hi_u32 profile                      = venc_create_chn_param->profile;
    hi_payload_type type                = venc_create_chn_param->type;
    hi_size venc_size                   = venc_create_chn_param->venc_size;
    ...
    if (type == HI_PT_MJPEG || type == HI_PT_JPEG) {
        venc_chn_attr->venc_attr.buf_size =
    ...
    if (gop_attr->gop_mode == HI_VENC_GOP_MODE_SMART_P) {
        venc_create_chn_param->stats_time = gop_attr->smart_p.bg_interval / venc_create_chn_param->gop;
    } else {
        venc_create_chn_param->stats_time = 1;
    }

    switch (type) {
        case HI_PT_H265:
            ret = sample_comm_venc_h265_param_init(venc_chn_attr, venc_create_chn_param);
            break;
        case HI_PT_H264:
            ret = sample_comm_venc_h264_param_init(venc_chn_attr, venc_create_chn_param);
            break;
        case HI_PT_MJPEG:
            ret = sample_comm_venc_mjpeg_param_init(venc_chn_attr, venc_create_chn_param);
            break;
        case HI_PT_JPEG:
            ret = sample_comm_venc_jpeg_param_init(venc_chn_attr);
            break;
        default:
            SAMPLE_PRT("cann't support this type (%d) in this version!\n", type);
            return HI_ERR_VENC_NOT_SUPPORT;
    }

    sample_comm_venc_set_gop_attr(type, venc_chn_attr, gop_attr);
...
hi_s32 sample_comm_venc_create(hi_venc_chn venc_chn, sample_comm_venc_chnl_param *venc_create_param)
{
    hi_s32 ret;
    hi_venc_chn_attr venc_chn_attr;
    hi_pic_size size = venc_create_param->size;

    //maohw venc_create_param->frame_rate           = 30; /* 30 is a number */
    //maohw venc_create_param->gop                  = 30; /* 30 is a number */

    if (sample_comm_sys_get_pic_size(size, &venc_create_param->venc_size) != HI_SUCCESS) {
        SAMPLE_PRT("get picture size failed!\n");
        return HI_FAILURE;
    }

    /* step 1:  create venc channel */
    ret = sample_comm_venc_channel_param_init(venc_create_param, &venc_chn_attr);
    if (ret != HI_SUCCESS) {
        SAMPLE_PRT("venc_channel_param_init failed!\n");
        return ret;
    }
    ...
    if(venc_create_param->type != HI_PT_JPEG)
    {
      ret = sample_comm_venc_close_reencode(venc_chn);
      if (ret != HI_SUCCESS) {
...
hi_s32 sample_comm_venc_start(hi_venc_chn venc_chn, sample_comm_venc_chnl_param *venc_create_param)
{
    ...
    ret = sample_comm_venc_create(venc_chn, venc_create_param);
    if (ret != HI_SUCCESS) {
    ...
    if(venc_create_param->type != HI_PT_JPEG)
    {
      ...
      if (ret != HI_SUCCESS) {
          SAMPLE_PRT("hi_mpi_venc_start_recv_pic faild with%#x! \n", ret);
          return HI_FAILURE;
      }
    }
    return HI_SUCCESS;
...
hi_s32 sample_comm_venc_stop(hi_venc_chn venc_chn)
{
    ...
    if (ret != HI_SUCCESS) {
        SAMPLE_PRT("hi_mpi_venc_stop_chn vechn[%d] failed with %#x!\n", venc_chn, ret);
    }

    /* distroy venc channel */
    ret = hi_mpi_venc_destroy_chn(venc_chn);
    if (ret != HI_SUCCESS) {
        SAMPLE_PRT("hi_mpi_venc_destroy_chn vechn[%d] failed with %#x!\n", venc_chn, ret);
        return HI_FAILURE;
    }
    return HI_SUCCESS;
}
    ...
    sample_comm_venc_frame_proc_info *addr_info)
{
    ...
    for (i = 0; (i < para->cnt) && (i < HI_VENC_MAX_CHN_NUM); i++) {
        ...
        if (ret != HI_SUCCESS) {
            SAMPLE_PRT("hi_mpi_sys_mmz_alloc err:0x%x", ret);
            return ret;
        }
        for (j = 0; (i < VENC_QPMAP_MAX_CHN) && (j < QPMAP_BUF_NUM); j++) {
            addr_info->qpmap_phys_addr[i][j] = phys_addr + j * addr_info->qpmap_size[i];
        ...
        for (j = 0; (i < VENC_QPMAP_MAX_CHN) && (j < QPMAP_BUF_NUM); j++) {
            addr_info->skip_weight_phys_addr[i][j] = phys_addr + j * addr_info->skip_weight_size[i];
    ...
    hi_venc_user_frame_info *frame_info, hi_video_frame_info *video_frame, hi_s32 index)
{
    ...
    if (ret != HI_SUCCESS) {
        SAMPLE_PRT("hi_mpi_venc_send_frame_ex err:0x%x\n", ret);

        ret = hi_mpi_vpss_release_chn_frame(para->vpss_grp, para->vpss_chn[index], video_frame);
        if (ret != HI_SUCCESS) {
            SAMPLE_PRT("hi_mpi_vpss_release_chn_frame err:0x%x", ret);
    ...
    sample_comm_venc_frame_proc_info *addr_info)
{
    ...
    hi_u32 frame_id = 0;
    hi_s32 ret;
    hi_video_frame_info *video_frame = HI_NULL;
    ...
    while (para->thread_start == HI_TRUE) {
        for (i = 0; (i < para->cnt) && (i < VENC_QPMAP_MAX_CHN); i++) {
            video_frame = &frame_info[i][frame_id].user_frame;
...
hi_void *sample_comm_qpmap_send_frame_proc(hi_void *p)
{
    ...
    if (para->cnt > VENC_QPMAP_MAX_CHN) {
        SAMPLE_PRT("Current func'sample_comm_qpmap_send_frame_proc' not support Venc channal num(%d) > %d\n", para->cnt,
            VENC_QPMAP_MAX_CHN);
        return HI_NULL;
    }
    ...
    for (i = 0; (i < para->cnt) && (i < HI_VPSS_MAX_PHYS_CHN_NUM); i++) {
        ...
        if (ret != HI_SUCCESS) {
            SAMPLE_PRT("hi_mpi_vpss_get_chn_attr err:0x%x", ret);
            goto error;
        }
        ...
        if (ret != HI_SUCCESS) {
            SAMPLE_PRT("hi_mpi_vpss_set_chn_attr err:0x%x", ret);
            goto error;
        }
    }
...
error:
    for (i = 0; (i < para->cnt) && (i < VENC_QPMAP_MAX_CHN); i++) {
        if (addr_info.qpmap_phys_addr[i][0] != 0) {
            ...
            if (ret != HI_SUCCESS) {
                SAMPLE_PRT("hi_mpi_sys_mmz_free err:0x%x", ret);
            }
        }
...
hi_void sample_venc_process_jpeg_roi(hi_void *virt_addr, hi_rect *rect, hi_u32 roi_level, hi_u32 stride)
{
    hi_s32 i;
    hi_u8 *wp = HI_NULL;
    hi_u32 start_x, start_y;
    hi_u32 level;
    hi_u32 start_block, start_byte, start_supple_idx, start_supple_block, start_block_height;
    hi_u32 write_block, write_resi_block, write_byte, write_tail_block, write_block_height;
    ...
    for (i = 0; i < write_block_height; i++) {
        wp = (hi_u8 *)virt_addr + (start_block_height + i) * stride + start_byte;
    ...
    hi_phys_addr_t *roimap_phys_addr)
{
    ...
    while (para->thread_start == HI_TRUE) {
        for (i = 0; (i < para->cnt) && (i < HI_VPSS_MAX_PHYS_CHN_NUM) && (i < HI_VENC_MAX_CHN_NUM); i++) {
            video_frame = &frame_info[i].user_frame;
            ret = hi_mpi_vpss_get_chn_frame(para->vpss_grp, para->vpss_chn[i],
                video_frame, 1000); /* 1000 is a number */
            if (ret != HI_SUCCESS) {
                SAMPLE_PRT("hi_mpi_vpss_get_chn_frame err: 0x%x\n", ret);
                continue;
            }
...
hi_void *sample_comm_send_roimap_frame_proc(hi_void *p)
{
    ...
    if (para->cnt > SAMPLE_VENC_ROIMAP_MAX_CHN) {
        SAMPLE_PRT("Current not support venc channal num(%d) > %d\n", para->cnt, SAMPLE_VENC_ROIMAP_MAX_CHN);
        return HI_NULL;
    }
    ...
    hi_s32 cnt, hi_size size[], hi_venc_jpeg_roi_attr roi_attr[])
{
    ...
    g_roimap_frame_param.cnt = cnt;

    for (i = 0; (i < cnt) && (i < HI_VENC_MAX_CHN_NUM) && (i < HI_VPSS_MAX_PHYS_CHN_NUM); i++) {
        g_roimap_frame_param.venc_chn[i] = venc_chn[i];
        g_roimap_frame_param.vpss_chn[i] = vpss_chn[i];
    ...
    sample_comm_venc_stream_proc_info *stream_proc_info)
{
    ...
    if (realpath(stream_proc_info->file_name[index], stream_proc_info->real_file_name[index]) == HI_NULL) {
        SAMPLE_PRT("chn[%d] stream file path error\n", venc_chn);
        return SAMPLE_RETURN_NULL;
    }
...
static hi_s32 sample_comm_set_name_save_stream(sample_comm_venc_stream_proc_info *stream_proc_info,
    hi_venc_stream_buf_info *stream_buf_info, sample_venc_getstream_para *para, hi_s32 venc_max_chn)
{
    hi_s32 i, ret, fd;
    hi_venc_chn_attr venc_chn_attr;
    ...
    for (i = 0; (i < stream_proc_info->chn_total) && (i < HI_VENC_MAX_CHN_NUM); i++) {
        ...
        if (ret != HI_SUCCESS) {
            SAMPLE_PRT("hi_mpi_venc_get_chn_attr chn[%d] failed with %#x!\n", venc_chn, ret);
            return SAMPLE_RETURN_NULL;
        }
        ...
        if (ret != HI_SUCCESS) {
            SAMPLE_PRT("sample_comm_venc_get_file_postfix [%d] failed with %#x!\n", venc_chn_attr.venc_attr.type, ret);
            return SAMPLE_RETURN_NULL;
        }
        ...
        /* set venc fd. */
        stream_proc_info->venc_fd[i] = hi_mpi_venc_get_fd(venc_chn); //maohw ???
        if (stream_proc_info->venc_fd[i] < 0) {
            SAMPLE_PRT("hi_mpi_venc_get_fd failed venc_chn:%d!\n", venc_chn);
            return SAMPLE_RETURN_NULL;
        }
        ...
        ret = hi_mpi_venc_get_stream_buf_info(venc_chn, &stream_buf_info[i]); //maohw;
        if (ret != HI_SUCCESS) {
            SAMPLE_PRT("hi_mpi_venc_get_stream_buf_info failed with %#x!\n", ret);
    ...
    hi_s32 index, hi_venc_stream_buf_info *stream_buf_info, hi_payload_type *payload_type)
{
    ...
    ret = hi_mpi_venc_query_status(stream_proc_info->venc_chn, &stat);//maohw;
    if (ret != HI_SUCCESS) {
        SAMPLE_PRT("hi_mpi_venc_query_status chn[%d] failed with %#x!\n", stream_proc_info->venc_chn, ret);
        return SAMPLE_RETURN_BREAK;
    }
    ...
    ret = hi_mpi_venc_get_stream(stream_proc_info->venc_chn, &stream, HI_TRUE);//maohw
    if (ret != HI_SUCCESS) {
        free(stream.pack);
        stream.pack = HI_NULL;
    ...
    ret = hi_mpi_venc_release_stream(stream_proc_info->venc_chn, &stream); //maohw
    if (ret != HI_SUCCESS) {
        SAMPLE_PRT("hi_mpi_venc_release_stream failed!\n");
        free(stream.pack);
        stream.pack = HI_NULL;
        return SAMPLE_RETURN_BREAK;
    }
...
hi_void *sample_comm_venc_get_venc_stream_proc(hi_void *p)
{
    ...
    fd_set read_fds;
    hi_s32 ret;
    hi_payload_type payload_type[HI_VENC_MAX_CHN_NUM] = {0};  // ???? maohw
    ...
    ret = sample_comm_set_name_save_stream(&stream_proc_info, stream_buf_info, para, HI_VENC_MAX_CHN_NUM);
...
hi_void sample_comm_venc_set_region(hi_u64 time, hi_venc_chn VeChn, hi_venc_svc_rect_info *pst_svc_rect)
{
    hi_s32 j;
    ...
    ret = hi_mpi_venc_send_svc_region(VeChn, pst_svc_rect);
    if (ret != HI_SUCCESS) {
        SAMPLE_PRT("Set hi_mpi_venc_send_svc_region failed for %#x chn =%d\n", ret, VeChn);
    }
}
hi_void sample_comm_venc_set_svc_param(hi_venc_chn VeChn, hi_venc_svc_param *pst_svc_param)
{
    hi_s32 j;
    ...
    hi_s32 ret;
    ret = hi_mpi_venc_get_svc_param(VeChn, pst_svc_param);
    if (ret != HI_SUCCESS) {
        SAMPLE_PRT("Set hi_mpi_venc_set_svc_param failed for %#x chn =%d\n", ret, VeChn);
    }
    ...
    ret = hi_mpi_venc_set_svc_param(VeChn, pst_svc_param);
    if (ret != HI_SUCCESS) {
        SAMPLE_PRT("Set hi_mpi_venc_set_svc_param failed for %#x!\n", ret);
    }
}
...
hi_void *sample_comm_venc_rateauto_stream_proc(hi_void *p)
{
    ...
    while (para->thread_start == HI_TRUE) {
        for (i = 0; (i < para->cnt) && (i < HI_VENC_MAX_CHN_NUM); i++) {
            ...
            if (ret != HI_SUCCESS) {
                SAMPLE_PRT("hi_mpi_vpss_get_chn_frame err:0x%x VeChn = %d\n", ret, para->venc_chn[i]);
                continue;
            }
            ret = hi_mpi_vpss_release_chn_frame(para->vpss_grp, para->vpss_chn[i], &video_frame);
            if (ret != HI_SUCCESS) {
                SAMPLE_PRT("hi_mpi_vpss_release_chn_frame err:0x%x\n", ret);
                return HI_NULL;
            }

            ret = hi_mpi_venc_enable_svc(para->venc_chn[i], HI_TRUE);
            if (ret != HI_SUCCESS) {
                SAMPLE_PRT("Set hi_mpi_venc_enable_svc failed for %#x!\n", ret);
                return HI_NULL;
            }
    ...
    hi_venc_stream_buf_info *stream_buf_info, hi_s32 venc_max_chn)
{
    ...
    for (i = 0; i < stream_proc_info->chn_total; i++) {
        ...
        if (stream_proc_info->venc_fd[i] < 0) {
            SAMPLE_PRT("hi_mpi_venc_get_fd failed with %#x!\n", stream_proc_info->venc_fd[i]);
            return SAMPLE_RETURN_NULL;
        }
        if (stream_proc_info->maxfd <= stream_proc_info->venc_fd[i]) {
            stream_proc_info->maxfd = stream_proc_info->venc_fd[i];
...
/******************************************************************************
* funciton : start rate auto process thread
******************************************************************************/
...
hi_s32 sample_comm_venc_stop_get_stream(hi_s32 chn_num)
{
    ...
    for (i = 0; i < chn_num; i++) {
        if (hi_mpi_venc_stop_chn(i) != HI_SUCCESS) {
            SAMPLE_PRT("chn %d hi_mpi_venc_stop_recv_pic failed!\n", i);
            return HI_FAILURE;
        }
    }
...
hi_s32 sample_comm_venc_stop_rateauto(hi_venc_chn venc_chn[], hi_s32 cnt)
{
    hi_s32 i;
    hi_s32 ret;
    ...
    if (g_venc_rateauto_frame_param.thread_start == HI_TRUE) {
        ...
        for (i = 0; i < cnt; i++) {
            VeChn = venc_chn[i];
            ret = hi_mpi_venc_enable_svc(VeChn, HI_TRUE);
            if (ret != HI_SUCCESS) {
                SAMPLE_PRT("Set hi_mpi_venc_enable_svc failed for %#x chn =%d\n", ret, VeChn);
            }
            if (memset_s(&svc_rect_info, sizeof(hi_venc_svc_rect_info), 0, sizeof(hi_venc_svc_rect_info)) != EOK) {
                printf("call memset error\n");
            }
            ret = hi_mpi_venc_send_svc_region(VeChn, &svc_rect_info);
            if (ret != HI_SUCCESS) {
                SAMPLE_PRT("Set hi_mpi_venc_send_svc_region failed for %#x chn =%d\n", ret, VeChn);
            }
            ret = hi_mpi_venc_get_svc_param(VeChn, &svc_param);
            if (ret != HI_SUCCESS) {
                SAMPLE_PRT("Set hi_mpi_venc_set_svc_param failed for %#x chn =%d\n", ret, VeChn);
            }
            ...
            ret = hi_mpi_venc_set_svc_param(VeChn, &svc_param);
            if (ret != HI_SUCCESS) {
                SAMPLE_PRT("Set hi_mpi_venc_set_svc_param failed for %#x chn =%d\n", ret, VeChn);
            }
            ret = hi_mpi_venc_enable_svc(VeChn, HI_FALSE);
            if (ret != HI_SUCCESS) {
                SAMPLE_PRT("Set hi_mpi_venc_enable_svc failed for %#x chn =%d\n", ret, VeChn);
            }
        }
...
hi_void sample_comm_venc_read_one_frame(FILE *fp, hi_u8 *y, hi_u8 *u, hi_u8 *v, hi_u32 width, hi_u32 height,
    hi_u32 stride, hi_u32 stride2)
{
    hi_u8 *dst = HI_NULL;
    hi_u32 row;
    dst = y;
    ...
    dst = u;
    for (row = 0; row < height / 2; row++) { /* 2 is a number */
        fread(dst, width / 2, 1, fp); /* 2 is a number */
        dst += stride2;
    }
    dst = v;
    for (row = 0; row < height / 2; row++) { /* 2 is a number */
...
hi_s32 sample_comm_venc_plan_to_semi(hi_u8 *u, hi_s32 u_stride, hi_u8 *v, hi_s32 pic_width, hi_s32 pic_height)
{
    hi_s32 i;
    hi_u8 *tmp_u = HI_NULL;
    hi_u8 *ptu = HI_NULL;
