...
typedef hi_s32 (*aac_encoder_open_callback)(hi_aac_encoder **ph_aac_plus_enc, hi_aacenc_config *config);
...
static hi_s32 aac_init_enc_lib(void)
{
    if (g_aac_enc_func.open_cnt == 0) {
        ...
        (hi_void)memcpy_s(&g_aac_enc_func, sizeof(aacenc_fun), &aac_enc_func, sizeof(aacenc_fun));
    }
...
hi_s32 aac_encoder_open_adp(hi_aac_encoder **ph_aac_plus_enc, hi_aacenc_config *config)
{
...
static hi_s32 aac_init_dec_lib(void)
{
    if (g_aac_dec_func.open_cnt == 0) {
        ...
        (hi_void)memcpy_s(&g_aac_dec_func, sizeof(aacdec_fun), &aac_dec_func, sizeof(aacdec_fun));
    }
...
static hi_s32 aenc_get_aaclc_bitrate(hi_aacenc_config *config, hi_s32 *min_rate,
                                     hi_s32 *max_rate, hi_s32 *recommemd_rate)
{
    ...
    if (config->sample_rate == HI_AUDIO_SAMPLE_RATE_32000) {
        ...
        *recommemd_rate = (chn_single) ? AACENC_BITRATE_48K : AACENC_BITRATE_128K;
    } else if (config->sample_rate == HI_AUDIO_SAMPLE_RATE_44100) {
        *min_rate = AACENC_BITRATE_48K;
        *max_rate = (chn_single) ? AACENC_BITRATE_265K : AACENC_BITRATE_320K;
        *recommemd_rate = (chn_single) ? AACENC_BITRATE_64K : AACENC_BITRATE_128K;
    } else if (config->sample_rate == HI_AUDIO_SAMPLE_RATE_48000) {
        *min_rate = AACENC_BITRATE_48K;
        *max_rate = (chn_single) ? AACENC_BITRATE_288K : AACENC_BITRATE_320K;
        *recommemd_rate = (chn_single) ? AACENC_BITRATE_64K : AACENC_BITRATE_128K;
    } else if (config->sample_rate == HI_AUDIO_SAMPLE_RATE_16000) {
        *min_rate = AACENC_BITRATE_24K;
        *max_rate = (chn_single) ? AACENC_BITRATE_96K : AACENC_BITRATE_192K;
        *recommemd_rate = AACENC_BITRATE_48K;
    } else if (config->sample_rate == HI_AUDIO_SAMPLE_RATE_8000) {
        *min_rate = AACENC_BITRATE_16K;
        *max_rate = (chn_single) ? AACENC_BITRATE_48K : AACENC_BITRATE_96K;
        *recommemd_rate = (chn_single) ? AACENC_BITRATE_24K : AACENC_BITRATE_32K;
    } else if (config->sample_rate == HI_AUDIO_SAMPLE_RATE_24000) {
        *min_rate = AACENC_BITRATE_32K;
        *max_rate = (chn_single) ? AACENC_BITRATE_144K : AACENC_BITRATE_288K;
        *recommemd_rate = AACENC_BITRATE_48K;
    } else if (config->sample_rate == HI_AUDIO_SAMPLE_RATE_22050) {
        *min_rate = AACENC_BITRATE_32K;
        *max_rate = (chn_single) ? AACENC_BITRATE_132K : AACENC_BITRATE_265K;
        *recommemd_rate = (chn_single) ? AACENC_BITRATE_64K : AACENC_BITRATE_48K;
    } else {
...
static hi_s32 aenc_check_aaclc_config(hi_aacenc_config *config)
{
...
static hi_s32 aenc_get_eaac_bitrate(hi_aacenc_config *config, hi_s32 *min_rate,
                                    hi_s32 *max_rate, hi_s32 *recommemd_rate)
{
    hi_bool chn_single = (config->num_channels_in == AACENC_CHANNEL_SINGLE) ? HI_TRUE : HI_FALSE;
    ...
    if (config->sample_rate == HI_AUDIO_SAMPLE_RATE_32000) {
        *min_rate = AACENC_BITRATE_32K;
        *max_rate = (chn_single) ? AACENC_BITRATE_64K : AACENC_BITRATE_128K;
        *recommemd_rate = (chn_single) ? AACENC_BITRATE_48K : AACENC_BITRATE_64K;
    } else if (config->sample_rate == HI_AUDIO_SAMPLE_RATE_44100) {
        *min_rate = AACENC_BITRATE_32K;
        *max_rate = (chn_single) ? AACENC_BITRATE_64K : AACENC_BITRATE_128K;
        *recommemd_rate = (chn_single) ? AACENC_BITRATE_48K : AACENC_BITRATE_64K;
...
static hi_s32 aenc_check_eaac_config(hi_aacenc_config *config)
{
    hi_s32 min_bit_rate = 0;
    hi_s32 max_bit_rate = 0;
    hi_s32 recommemd_rate = 0;
...
static hi_s32 aenc_get_eaacplus_bitrate(hi_aacenc_config *config, hi_s32 *min_rate,
                                        hi_s32 *max_rate, hi_s32 *recommemd_rate)
{
    if (config->sample_rate == HI_AUDIO_SAMPLE_RATE_32000) {
        *min_rate = AACENC_BITRATE_16K;
...
static hi_s32 aenc_check_eaacplus_config(hi_aacenc_config *config)
{
    hi_s32 min_bit_rate = 0;
    hi_s32 max_bit_rate = 0;
    hi_s32 recommemd_rate = 0;
...
static hi_s32 aenc_get_aacld_bitrate(hi_aacenc_config *config, hi_s32 *min_rate,
                                     hi_s32 *max_rate, hi_s32 *recommemd_rate)
{
    hi_bool chn_single = (config->num_channels_in == AACENC_CHANNEL_SINGLE) ? HI_TRUE : HI_FALSE;

    if (config->sample_rate == HI_AUDIO_SAMPLE_RATE_32000) {
        *min_rate = (chn_single) ? AACENC_BITRATE_48K : AACENC_BITRATE_64K;
        *max_rate = AACENC_BITRATE_320K;
        *recommemd_rate = (chn_single) ? AACENC_BITRATE_64K : AACENC_BITRATE_128K;
    } else if (config->sample_rate == HI_AUDIO_SAMPLE_RATE_44100) {
        *min_rate = (chn_single) ? AACENC_BITRATE_64K : AACENC_BITRATE_44K;
        *max_rate = AACENC_BITRATE_320K;
        *recommemd_rate = (chn_single) ? AACENC_BITRATE_128K : AACENC_BITRATE_256K;
    } else if (config->sample_rate == HI_AUDIO_SAMPLE_RATE_48000) {
        ...
        *max_rate = (chn_single) ? AACENC_BITRATE_192K : AACENC_BITRATE_320K;
        *recommemd_rate = (chn_single) ? AACENC_BITRATE_48K : AACENC_BITRATE_96K;
    } else if (config->sample_rate == HI_AUDIO_SAMPLE_RATE_8000) {
        *min_rate = AACENC_BITRATE_16K;
        *max_rate = (chn_single) ? AACENC_BITRATE_96K : AACENC_BITRATE_192K;
        *recommemd_rate = (chn_single) ? AACENC_BITRATE_24K : AACENC_BITRATE_48K;
    } else if (config->sample_rate == HI_AUDIO_SAMPLE_RATE_24000) {
        *min_rate = (chn_single) ? AACENC_BITRATE_32K : AACENC_BITRATE_48K;
        *max_rate = (chn_single) ? AACENC_BITRATE_256K : AACENC_BITRATE_320K;
        *recommemd_rate = (chn_single) ? AACENC_BITRATE_64K : AACENC_BITRATE_128K;
    } else if (config->sample_rate == HI_AUDIO_SAMPLE_RATE_22050) {
        *min_rate = (chn_single) ? AACENC_BITRATE_32K : AACENC_BITRATE_48K;
        *max_rate = (chn_single) ? AACENC_BITRATE_256K : AACENC_BITRATE_320K;
...
static hi_s32 aenc_check_aacld_config(hi_aacenc_config *config)
{
    hi_s32 min_bit_rate = 0;
    hi_s32 max_bit_rate = 0;
    hi_s32 recommemd_rate = 0;
...
static hi_s32 aenc_get_aaceld_bitrate(hi_aacenc_config *config, hi_s32 *min_rate,
                                      hi_s32 *max_rate, hi_s32 *recommemd_rate)
{
    hi_bool chn_single = (config->num_channels_in == AACENC_CHANNEL_SINGLE) ? HI_TRUE : HI_FALSE;

    if (config->sample_rate == HI_AUDIO_SAMPLE_RATE_32000) {
        *min_rate = (chn_single) ? AACENC_BITRATE_32K : AACENC_BITRATE_64K;
        *max_rate = AACENC_BITRATE_320K;
        *recommemd_rate = (chn_single) ? AACENC_BITRATE_64K : AACENC_BITRATE_128K;
    } else if (config->sample_rate == HI_AUDIO_SAMPLE_RATE_44100) {
        *min_rate = (chn_single) ? AACENC_BITRATE_96K : AACENC_BITRATE_192K;
        *max_rate = AACENC_BITRATE_320K;
        *recommemd_rate = (chn_single) ? AACENC_BITRATE_128K : AACENC_BITRATE_256K;
    } else if (config->sample_rate == HI_AUDIO_SAMPLE_RATE_48000) {
        *min_rate = (chn_single) ? AACENC_BITRATE_96K : AACENC_BITRATE_192K;
        ...
        *min_rate = (chn_single) ? AACENC_BITRATE_16K : AACENC_BITRATE_32K;
        *max_rate = (chn_single) ? AACENC_BITRATE_256K : AACENC_BITRATE_320K;
        *recommemd_rate = (chn_single) ? AACENC_BITRATE_48K : AACENC_BITRATE_96K;
    } else if (config->sample_rate == HI_AUDIO_SAMPLE_RATE_8000) {
        *min_rate = (chn_single) ? AACENC_BITRATE_32K : AACENC_BITRATE_64K;
        *max_rate = (chn_single) ? AACENC_BITRATE_96K : AACENC_BITRATE_192K;
        *recommemd_rate = (chn_single) ? AACENC_BITRATE_32K : AACENC_BITRATE_64K;
    } else if (config->sample_rate == HI_AUDIO_SAMPLE_RATE_24000) {
        *min_rate = (chn_single) ? AACENC_BITRATE_24K : AACENC_BITRATE_32K;
        *max_rate = (chn_single) ? AACENC_BITRATE_256K : AACENC_BITRATE_320K;
        *recommemd_rate = (chn_single) ? AACENC_BITRATE_64K : AACENC_BITRATE_128K;
...
static hi_s32 aenc_check_aaceld_config(hi_aacenc_config *config)
{
    hi_s32 min_bit_rate = 0;
    hi_s32 max_bit_rate = 0;
    hi_s32 recommemd_rate = 0;
...
hi_s32 aenc_aac_check_config(hi_aacenc_config *config)
{
    hi_s32 ret = 0;
    ...
    if (config->coder_format != HI_AACLC && config->coder_format != HI_EAAC &&
        config->coder_format != HI_EAACPLUS && config->coder_format != HI_AACLD &&
        config->coder_format != HI_AACELD) {
...
hi_s32 open_aac_encoder(hi_void *encoder_attr, hi_void **pp_encoder)
{
    ...
    *pp_encoder = (hi_void *)encoder;
    ...
    ret = memcpy_s(&encoder->aac_attr, sizeof(hi_aenc_attr_aac), attr, sizeof(hi_aenc_attr_aac));
    if (ret != EOK) {
    ...
    hi_s16 *in_data, hi_s16 in_len)
{
     ...
     */
    if (aac_encoder->aac_attr.snd_mode == HI_AUDIO_SOUND_MODE_STEREO) {
        if ((in_len / 2) < (water_line * 2)) { /* 2: 16bit */
        ...
        if ((in_len / 2) < water_line) { /* 2: 16bit */
            printf("[func]:%s [line]:%d [info]:%s\n", __FUNCTION__, __LINE__, "invalid in_len");
            return HI_ERR_AENC_ILLEGAL_PARAM;
        }
        for (i = water_line - 1; i >= 0; i--) {
...
hi_s32 open_aac_decoder(hi_void *decoder_attr, hi_void **pp_decoder)
{
    ...
    *pp_decoder = (hi_void *)decoder;
    ...
    ret = memcpy_s(&decoder->aac_attr, sizeof(hi_adec_attr_aac), attr, sizeof(hi_adec_attr_aac));
    if (ret != EOK) {
                      ...
                      hi_u16 *outbuf, hi_u32 *out_len, hi_u32 *chns)
{
    ...
    *chns = 1; /* voice encoder only one channle */
