...
static void* sample_task(void* p);
...
int sample_start()
{
  ...
  return pthread_create(&s_hMdThread, NULL, sample_task, NULL);
}
...
static void* sample_task(void* p)
{
  ...
  while (HI_FALSE == s_bStopSignal)
  {
      ...
      printf("HI_MPI_VPSS_GetChnFrame OK, VPSS_GRP(%d), VPSS_CHN(%d), (%d x %d), PTS:%llu\n",
                  s32VpssGrp, as32VpssChn[1], 
                  stExtFrmInfo.stVFrame.u32Width, stExtFrmInfo.stVFrame.u32Height,
                  stExtFrmInfo.stVFrame.u64PTS/1000);
