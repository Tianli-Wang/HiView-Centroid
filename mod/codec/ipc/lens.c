...
static int __uart_open(int ch, char *ttyAMA, int baudrate)  { return -1;}
static int __uart_write(int ch, unsigned char *buf, int size){ return -1;}
...
int (*gsf_uart_open)(int ch, char *ttyAMA, int baudrate) = __uart_open;
int (*gsf_uart_write)(int ch, unsigned char *buf, int size) = __uart_write;
